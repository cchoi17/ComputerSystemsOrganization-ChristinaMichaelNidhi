...
int main() {
   ...
   char randomWord[wordLength];
   strncpy(randomWord, words[index], sizeof(words[index]));

   bool mask[wordLength];
   mask[0] = FALSE;
  ...
  // Loop over each round of guessing
   while( allGuessed != true ) {
     ...
     // Get player's next guess
      ...
      for( int j = 0; j < wordLength; j++ ) {
        if(guess == randomWord[j] && mask[j] != true) {
            mask[j] = true;
            guesses++;
        }
      }
      printf("You have guessed %d char", guesses);
